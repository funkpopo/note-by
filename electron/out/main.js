...
function createWindow() {
    ...
    electron_1.ipcMain.handle('save-ai-config', async (_, config) => {
        return await (0, ai_config_1.saveAIConfig)(config);
    });
    // 处理AI提示词调用
    electron_1.ipcMain.handle('call-ai-with-prompt', async (_, promptType, promptId, selectedContent) => {
        var _a, _b, _c, _d;
        try {
            // 获取AI配置
            const aiConfig = await (0, ai_config_1.getAIConfig)();
            if (!aiConfig.success) {
                return {
                    success: false,
            ...
            // 检查提示词类型是否有效
            if (!aiConfig.prompts || !aiConfig.prompts[promptType]) {
                return {
                    success: false,
            ...
            const promptGroup = aiConfig.prompts[promptType];
            let targetPrompt = promptGroup.find((p) => p.id === promptId);
            // 如果找不到指定ID的提示词，则使用默认提示词
            if (!targetPrompt) {
                targetPrompt = promptGroup.find((p) => p.isDefault) || promptGroup[0];
            }
            if (!targetPrompt) {
                return {
                    success: false,
            ...
            const finalPrompt = targetPrompt.prompt.replace('{{content}}', selectedContent);
            // 构建请求体
            const requestBody = {
            ...
            if (!response.ok) {
                ...
                return {
                    success: false,
                    error: `API请求失败: ${response.status} ${response.statusText} - ${((_a = errorData.error) === null || _a === void 0 ? void 0 : _a.message) || JSON.stringify(errorData)}`
                };
            }
            const data = await response.json();
            // 假设返回格式是OpenAI兼容的
            const responseContent = ((_d = (_c = (_b = data.choices) === null || _b === void 0 ? void 0 : _b[0]) === null || _c === void 0 ? void 0 : _c.message) === null || _d === void 0 ? void 0 : _d.content) || '';
