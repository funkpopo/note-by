...
export async function getRAGDocument(
  ...
  try {
    ...
    const stmt = database.prepare(`
      SELECT id, file_path as filePath, title, content, content_hash as contentHash,
...
export async function addDocumentChunk(
  chunk: Omit<DocumentChunk, 'id' | 'createdAt'>
): Promise<number | null> {
  try {
    const database = await initDatabase()
    if (!database) return null
    ...
    const stmt = database.prepare(`
      ...
      (document_id, chunk_index, content, start_position, end_position, token_count, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `)
  ...
  try {
    ...
    const stmt = database.prepare(`
      SELECT id, document_id as documentId, chunk_index as chunkIndex, content,
             start_position as startPosition, end_position as endPosition,
...
// 删除文档的所有分块
export async function deleteDocumentChunks(documentId: number): Promise<boolean> {
  try {
    const database = await initDatabase()
    if (!database) return false
...
export async function addDocumentEmbedding(
  embedding: Omit<DocumentEmbedding, 'id' | 'createdAt'>
): Promise<number | null> {
  try {
    const database = await initDatabase()
    if (!database) return null
...
// 获取分块的向量
export async function getDocumentEmbedding(chunkId: number): Promise<DocumentEmbedding | null> {
  try {
    const database = await initDatabase()
    if (!database) return null

    const stmt = database.prepare(`
      SELECT id, chunk_id as chunkId, embedding, embedding_model as embeddingModel, created_at as createdAt
      FROM document_embeddings WHERE chunk_id = ?
    `)
    return stmt.get(chunkId) as DocumentEmbedding | null
  } catch (error) {
    return null
  }
}
...
// 获取所有向量（用于相似度搜索）
export async function getAllDocumentEmbeddings(
  embeddingModel?: string
): Promise<Array<DocumentEmbedding & { documentId: number; content: string; filePath: string }>> {
  try {
    const database = await initDatabase()
    if (!database) return []

    let query = `
      SELECT e.id, e.chunk_id as chunkId, e.embedding, e.embedding_model as embeddingModel,
    ...
    if (embeddingModel) {
      query += ' WHERE e.embedding_model = ?'
      const stmt = database.prepare(query)
      return stmt.all(embeddingModel) as Array<
        DocumentEmbedding & { documentId: number; content: string; filePath: string }
      >
    } else {
      const stmt = database.prepare(query)
